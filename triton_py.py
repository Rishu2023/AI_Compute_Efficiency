# -*- coding: utf-8 -*-
"""Triton Optimized Matrix Multiplication for NVIDIA T4 GPU.

High-performance FP16 Tensor Core matmul kernel with:
- L2-friendly grouped tile ordering for cache locality
- Maskless inner loop via K-padding (zero branch overhead)
- Aggressive autotuning configs sized for T4's 64 KB shared memory
- Software pipelining via multi-stage buffering
...
def get_configs():
    return [
        ...
        # Additional T4-optimized configs for various matrix shapes
...
def matmul_kernel(
    ...
    """Optimized FP16 matmul kernel targeting T4 Tensor Cores.

    Uses L2-friendly grouped tile ordering and maskless K-loop
    (K is pre-padded to a multiple of BLOCK_K in the Python wrapper).
    """
...
def triton_matmul(a_fp16: torch.Tensor, b_fp16: torch.Tensor) -> torch.Tensor:
    """Compute C = A @ B using optimized Triton kernel.

    Args:
        a_fp16: [M, K] fp16 contiguous tensor
        b_fp16: [K, N] fp16 contiguous tensor
...
triton_matmul_v4 = triton_matmul
...
def benchmark(M, N, K):
    """Benchmark Triton vs PyTorch matmul at a given size."""
    ...
    t_ms = bench(lambda: triton_matmul(a16, b16))
    pt_ms = bench(lambda: torch.matmul(a16, b16))
...
def verify():
    """Verify correctness of Triton matmul against PyTorch."""
    ...
    print(f"Correctness — max|diff|: {err:.5f}  "
